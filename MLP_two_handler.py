...
class CustomImageInput(ImageClassifier):
    ...
    def preprocess(self, data: list) -> torch.float32:
        ...
        #Catch data from MLP one 
        self.mlp_one_response = data[0].get("MLP_one").decode()
        
        #Catch data from common_preprocessing
        images = self.get_json_from_bytesarray(data)
       
        image = base64.b64decode(images["image_two"])
    ...
    def inference(self, data, *args, **kwargs):
        ...
        with torch.no_grad():
            ...
            data = self.model(marshalled_data).cpu().detach().tolist()[0]
        return data
